...
async def login_form(email: str = Form(...), password: str = Form(...)) -> dict:
...
async def refresh_token(
    refresh_token: str = Form(...), session: AsyncSession = Depends(get_async_session)
) -> Token:
    ...
    # refresh_token не обновляется, действует до конца периода, а потом разлогин
    return Token(access_token=access_token, refresh_token=refresh_token)
